...
public class Version13RandomProbe extends TlsClientProbe<ClientScannerConfig, ClientReport> {
	...
	private boolean testIfDownGradeEnforcedProtocolVersion(ProtocolVersion version) {
		...
		config.setDefaultServerSupportedCipherSuites(
				CipherSuite.getImplemented().stream().filter(suite -> !suite.isTLS13()).collect(Collectors.toList()));

		// patch randomness
		byte[] serverRandomPostfix = version == ProtocolVersion.TLS12 ? SERVER_RANDOM_12_POSTFIX
		...
		return WorkflowTraceUtil.didReceiveMessage(HandshakeMessageType.CERTIFICATE, state.getWorkflowTrace());
	}
	...
	protected Requirement getRequirements() {
		...
		return new OrRequirement(tls10, tls11, tls12).requires(new ProbeRequirement(TlsProbeType.PROTOCOL_VERSION))
