...
public class ClientScannerConfig extends TlsScannerConfig {
	...
	@ParametersDelegate
	private ClientParameterDelegate clientParameterDelegate;

	@Parameter(names = "-run", required = true, description = "The shell command the scanner should run to start the client. The port number the client should connect to must be marked with '"
			+ PORT_REPLACEMENT_MARKER + "'")
	private String runCommand = null;

	@Parameter(names = "-runDirectory", required = false, description = "The working directory the scanner should use when starting the client")
	private String runDirectory = null;

	@Parameter(names = "-logDirectory", required = false, description = "The directory the scanner should use to log the client output. Be wary: This creates a lot of files for a full scan")
	...
	/**
	 * Retrieves the socket listening port from a given server transport handler. In
	 * case of dynamic port allocation, this returns the port assigned to the server
	 * socket.
	 */
	private Integer getServerPort(TransportHandler serverTransportHandler) {
		if (serverTransportHandler == null) {
			throw new RuntimeException("ServerTransportHandler was null when trying to extract server port.");
		}
		...
		throw new RuntimeException("Got unknown ServerTransportHandler when trying to extract server port.");
	}
}
