...
public class ClientContainerReportCreator extends TlsReportCreator {
	...
	protected ReportContainer createDtlsCookieContainer(TlsScanReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_HVR_LEGACY_SERVER_VERSION_MISMATCH, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_HVR_RECORD_SEQUENCE_NUMBER_MISMATCH, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_SERVER_HELLO_RECORD_SEQUENCE_NUMBER_MISMATCH,
				report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_CLIENT_HELLO_MISMATCH, report));
	...
	private ListContainer createSupportedCipherSuitesContainer(ClientReport report) {
		...
		for (CipherSuite suite : CollectionUtils.mergeCollectionsIntoSet(getRealClientAdvertisedCipherSuites(report),
	...
	private List<CipherSuite> getRealClientAdvertisedCipherSuites(ClientReport report) {
		return report.getClientAdvertisedCipherSuites().stream().filter(suite -> suite.isRealCipherSuite())
	...
	private ReportContainer createSessionResumptionContainer(ClientReport report) {
		...
		if (report.getExecutedProbes().contains(TlsProbeType.RESUMPTION)) {
			...
			container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_SESSION_ID_RESUMPTION, report));
			if (report.getProtocolType() == ProtocolType.DTLS) {
				container.add(createKeyValueContainer(
						TlsAnalyzedProperty.SUPPORTS_DTLS_COOKIE_EXCHANGE_IN_SESSION_ID_RESUMPTION, report));
			}
			container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_SESSION_TICKET_RESUMPTION, report));
			if (report.getProtocolType() == ProtocolType.DTLS) {
				container.add(createKeyValueContainer(
						TlsAnalyzedProperty.SUPPORTS_DTLS_COOKIE_EXCHANGE_IN_SESSION_TICKET_RESUMPTION, report));
			}
		}
		return container;
	}
	...
	private ReportContainer createAttackVulnerabilitiesContainer(ClientReport report) {
		...
		if (report.getKnownPaddingOracleVulnerability() != null) {
			container.add(new TextContainer("- " + report.getKnownPaddingOracleVulnerability().getShortName(),
					AnsiColor.RED));
		}
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_FREAK_DOWNGRADE, report));
	...
	private ReportContainer createDheParameterContainer(ClientReport report) {
		...
		if (lowestPossibleDheModulusSize != null) {
			...
			container.add(new KeyValueContainer(containerKey, AnsiColor.DEFAULT_COLOR, containerValue,
	...
	private ReportContainer createQuirksContainer(ClientReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.TLS_1_3_DOWNGRADE_PROTECTION, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.SENDS_APPLICATION_MESSAGE, report));
		return container;
	}
	...
	private ReportContainer createAlpnContainer(ClientReport report) {
		ListContainer container = new ListContainer();
		if (report.getExecutedProbes().contains(TlsProbeType.ALPN) && report.getClientAdvertisedAlpns() != null) {
	...
	private ReportContainer createPaddingOracleContainer(ClientReport report) {
		...
		if (Objects.equals(report.getResult(TlsAnalyzedProperty.VULNERABLE_TO_PADDING_ORACLE), TestResults.TRUE)) {
			container.add(createPaddingOracleKnownVulnerabilityContainer(report.getKnownPaddingOracleVulnerability()));
		}
		if (report.getPaddingOracleTestResultList() == null || report.getPaddingOracleTestResultList().isEmpty()) {
			...
			appendInformationLeakTestList(container, informationLeakTestList, "Padding Oracle Response Map");
		}
		return container;
	}
	...
	private ListContainer createPaddingOracleKnownVulnerabilityContainer(
		...
		if (knownVulnerability == null) {
			container.add(new KeyValueContainer("Identification", AnsiColor.DEFAULT_COLOR,
					"Could not identify vulnerability. Please contact us if you know which software/hardware is generating this behavior.",
					AnsiColor.YELLOW));
			return container;
		}
		container.add(new KeyValueContainer("Identification", AnsiColor.DEFAULT_COLOR, knownVulnerability.getLongName(),
				AnsiColor.RED));
		container
				.add(new KeyValueContainer("CVE", AnsiColor.DEFAULT_COLOR, knownVulnerability.getCve(), AnsiColor.RED));
		AnsiColor oracleStrengthColor = knownVulnerability.getStrength() != PaddingOracleStrength.WEAK ? AnsiColor.RED
				: AnsiColor.YELLOW;
		container.add(new KeyValueContainer("Strength", AnsiColor.DEFAULT_COLOR,
				knownVulnerability.getStrength().name(), oracleStrengthColor));
		AnsiColor oracleObservableColor = knownVulnerability.isObservable() ? AnsiColor.RED : AnsiColor.YELLOW;
		container.add(new KeyValueContainer("Observable", AnsiColor.DEFAULT_COLOR,
				String.valueOf(knownVulnerability.isObservable()), oracleObservableColor));
		...
		container.add(createDefaultTextContainer(
				"If your tested software/hardware is not in this list, please let us know so we can add it here."));

		return container;
	}

	private void appendInformationLeakTestList(ListContainer outerContainer,
			List<InformationLeakTest> informationLeakTestList, String title) {
		...
		LinkedList<TextContainer> headline = informationLeakTestList.get(0).getTestInfo().getFieldNames().stream()
				.map(this::createDefaultTextContainer).collect(Collectors.toCollection(LinkedList::new));
		...
		for (InformationLeakTest testResult : informationLeakTestList) {
			String valueP = testResult.getValueP() >= 0.001 ? String.format("%.3f", testResult.getValueP()) : "<0.001";
			List<String> resultStrings = Arrays.asList(testResult.getTestInfo().getTechnicalName().split(":"));
			if (testResult.getValueP() < 0.01) {
				List<TextContainer> tableLine = resultStrings.stream().map(x -> new TextContainer(x, AnsiColor.RED))
						.collect(Collectors.toCollection(LinkedList::new));
				tableLine.add(new TextContainer(testResult.getEqualityError().name(), AnsiColor.RED));
				...
				List<TextContainer> tableLine = resultStrings.stream().map(x -> new TextContainer(x, AnsiColor.YELLOW))
						.collect(Collectors.toCollection(LinkedList::new));
				tableLine.add(new TextContainer(testResult.getEqualityError().name(), AnsiColor.YELLOW));
				...
				List<TextContainer> tableLine = resultStrings.stream().map(x -> new TextContainer(x, AnsiColor.GREEN))
						.collect(Collectors.toCollection(LinkedList::new));
	...
	private void appendInformationLeakTestResult(InformationLeakTest informationLeakTest,
		...
		for (VectorContainer vectorContainer : vectorContainerList) {
			...
			responseTable.setHeadlineList(List.of(new TextContainer("Fingerprint", AnsiColor.DEFAULT_COLOR),
			...
			for (ResponseCounter counter : vectorContainer.getDistinctResponsesCounterList()) {
				AnsiColor color = counter.getFingerprint().equals(defaultAnswer) ? AnsiColor.GREEN : AnsiColor.RED;
				...
				tableLine.add(new TextContainer(String.format("%.2f", counter.getProbability() * 100) + "%", color));
			}
		}
	}
