...
public class CertificateProbe extends TlsClientProbe<ClientScannerConfig, ClientReport> {
	...
	public void executeTest() {
		...
		for (ClientCertificateType certType : getTestableCertTypes()) {
			...
			CertificateChain clientCertChain = new CertificateChain(clientCert,
					config.getDefaultServerConnection().getHostname());
	...
	private Config getConfig(ClientCertificateType clientCertType) {
		...
		List<CipherSuite> suitableCipherSuites = config.getDefaultServerSupportedCipherSuites().stream()
				.filter(suite -> isCipherSuiteSuitableForCertType(suite, clientCertType)).collect(Collectors.toList());
	...
	private boolean isCipherSuiteSuitableForCertType(CipherSuite cipherSuite, ClientCertificateType clientCertType) {
		...
		default:
			throw new UnsupportedOperationException(
					"Client certificate type " + clientCertType.name() + " is not supported yet");
		}
	}
	...
	private Certificate getClientCertificate(Config config, ClientCertificateType certType) {
		...
		WorkflowTraceMutator.replaceSendingMessage(trace, HandshakeMessageType.CERTIFICATE_REQUEST, message);
	...
	private boolean isCertificateAlreadyIncluded(Set<CertificateChain> recordedCertificates,
			CertificateChain certificate) {
		return recordedCertificates.stream().anyMatch(currCertChain -> currCertChain.getCertificateReportList()
