...
public class PaddingOracleProbe extends TlsClientProbe<ClientScannerConfig, ClientReport> {
	...
	public PaddingOracleProbe(ParallelExecutor parallelExecutor, ClientScannerConfig scannerConfig) {
		super(parallelExecutor, TlsProbeType.PADDING_ORACLE, scannerConfig);
		this.scanDetail = scannerConfig.getScanDetail();
		this.numberOfIterations = scanDetail.isGreaterEqualTo(ScannerDetail.NORMAL) ? NUMBER_OF_ITERATIONS
				: NUMBER_OF_ITERATIONS_IN_QUICK_MODE;
		this.numberOfAddtionalIterations = scanDetail.isGreaterEqualTo(ScannerDetail.NORMAL)
				? NUMBER_OF_ADDTIONAL_ITERATIONS
				: NUMBER_OF_ADDTIONAL_ITERATIONS_IN_QUICK_MODE;
		register(TlsAnalyzedProperty.VULNERABLE_TO_PADDING_ORACLE, TlsAnalyzedProperty.PADDINGORACLE_TEST_RESULT);
	}
				...
				if (!pair.getVersion().isSSL() && !pair.getVersion().isTLS13()) {
					for (CipherSuite suite : pair.getCipherSuiteList()) {
						if (!suite.isPsk() && suite.isCBC() && CipherSuite.getImplemented().contains(suite)) {
							PaddingRecordGeneratorType recordGeneratorType = scanDetail
									.isGreaterEqualTo(ScannerDetail.NORMAL) ? PaddingRecordGeneratorType.SHORT
	...
	private InformationLeakTest<PaddingOracleTestInfo> getPaddingOracleInformationLeakTest(
		...
		PaddingOracleAttacker attacker = new PaddingOracleAttacker(scannerConfig.createConfig(), getParallelExecutor(),
	...
	private void extendFingerPrint(InformationLeakTest<PaddingOracleTestInfo> informationLeakTest,
		...
		InformationLeakTest<PaddingOracleTestInfo> intermediateResponseMap = getPaddingOracleInformationLeakTest(
	...
	public void adjustConfig(ClientReport report) {
		...
		sendsApplicationMessage = report.getResult(TlsAnalyzedProperty.SENDS_APPLICATION_MESSAGE) == TestResults.TRUE;
	}
