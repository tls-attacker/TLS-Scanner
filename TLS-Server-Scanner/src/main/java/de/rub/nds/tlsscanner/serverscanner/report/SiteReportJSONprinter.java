...
public class SiteReportJSONprinter {
	...
	public ObjectNode appendHsNormal(ObjectNode jsonReport) {
		...
		handShakeSimulationOverview.put("Handshakes - Successful",
	...
	public ObjectNode appendHandshakeSimulationDetails(ObjectNode jsonReport) {
		for (SimulatedClientResult simulatedClient : report.getSimulatedClientList()) {
			...
			simulatedCientNode.put("Selected Ciphersuite", simulatedClient.getSelectedCipherSuite().name());
			...
			if (simulatedClient.getSelectedCompressionMethod() != null) {
				simulatedCientNode.put("Selected Compression Method",
	...
	public ObjectNode appendRenegotiation(ObjectNode jsonReport) {
		...
		renegotioation.put("Secure (Extension)",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CLIENT_SIDE_SECURE_RENEGOTIATION_EXTENSION)));
		renegotioation.put("Secure (CipherSuite)", String
				.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CLIENT_SIDE_SECURE_RENEGOTIATION_CIPHERSUITE)));
			...
			for (int i = 0; i < chain.getCertificateReportList().size(); i++) {
				...
				if (certReport.getValidFrom() != null && certReport.getValidTo() != null
					...
					if (days < 1) {
						...
						cert.put("Expires in", days + " days. This is usually too long for a leaf certificate");
					} else {
				...
				if (certReport.getSignatureAndHashAlgorithm() != null) {
					if (certReport.getSignatureAndHashAlgorithm().getHashAlgorithm() == HashAlgorithm.SHA1
						...
						cert.put("Hash Algorithm", certReport.getSignatureAndHashAlgorithm().getHashAlgorithm().name());
					}
				}
	...
	private ObjectNode appendOcspOverview(ObjectNode jsonReport) {
		...
		if (report.getResult(AnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST) == TestResult.TRUE
			...
			ocsp.put("OCSPStapling",
					"OCSP Stapling is unreliable on this server.Extension scan reported OCSP Stapling support, but OCSP scan does not.The results are likely incomplete. Maybe rescan for more information?");
			...
			ocsp.put("OCSPStapling",
					"OCSP Stapling is unreliable on this server.Extension scan reported no OCSP support, but OCSP scan does.");
			report.putResult(AnalyzedProperty.STAPLING_UNRELIABLE, TestResult.TRUE);
		}
		...
		if (report.getResult(AnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST_TLS13) != TestResult.COULD_NOT_TEST) {
			ocsp.put("OCSP Stapling (TLS 1.3)",
					String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST_TLS13)));
			ocsp.put("Multi Stapling (TLS 1.3)",
					String.valueOf(report.getResult(AnalyzedProperty.STAPLING_TLS13_MULTIPLE_CERTIFICATES)));
		}

		if (Boolean.TRUE.equals(report.getResult(AnalyzedProperty.SUPPORTS_NONCE) == TestResult.TRUE)) {
			ocsp.put("Nonce Mismatch / Cached Nonce",
					String.valueOf(report.getResult(AnalyzedProperty.NONCE_MISMATCH)));
		}
		...
		if (report.getResult(AnalyzedProperty.SUPPORTS_OCSP_STAPLING) == TestResult.TRUE) {
			ocsp.put("Includes Stapled Response",
					String.valueOf(report.getResult(AnalyzedProperty.INCLUDES_CERTIFICATE_STATUS_MESSAGE)));
			ocsp.put("Stapled Response Expired",
					String.valueOf(report.getResult(AnalyzedProperty.STAPLED_RESPONSE_EXPIRED)));
		}
	...
	private ObjectNode appendOcspForCertificate(OcspCertificateResult result, int certNo) {
		...
		ocspDetailed.put("certNumber",
				"Detailed OCSP results for certificate " + certNo + " of " + report.getOcspResults().size());
		...
		if (result.getFirstResponse() != null) {
			if (result.getFirstResponse().getResponseStatus() > 0) {
				ocspDetailed.put("Requested OCSP Response (HTTP POST)",
		...
		if (result.getHttpGetResponse() != null) {
			...
			ocspDetailed.put("Requested OCSP Response (HTTP GET)",
	...
	public ObjectNode appendSession(ObjectNode jsonReport) {
		...
		session.put("Supports Session resumption",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_SESSION_IDS)));
		session.put("Supports Session Tickets",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_SESSION_TICKETS)));
		session.put("Issues TLS 1.3 Session Tickets",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS13_SESSION_TICKETS)));
	...
	public ObjectNode appendIntolerances(ObjectNode jsonReport) {
		...
		commonBugs.put("Version Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_VERSION_INTOLERANCE)));
		commonBugs.put("Ciphersuite Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_CIPHER_SUITE_INTOLERANCE)));
		commonBugs.put("Extension Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_EXTENSION_INTOLERANCE)));
		commonBugs.put("CS Length Intolerant (>512 Byte)",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_CIPHER_SUITE_LENGTH_INTOLERANCE)));
		commonBugs.put("Compression Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_COMPRESSION_INTOLERANCE)));
		commonBugs.put("ALPN Intolerant", String.valueOf(report.getResult(AnalyzedProperty.HAS_ALPN_INTOLERANCE)));
		commonBugs.put("CH Length Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_CLIENT_HELLO_LENGTH_INTOLERANCE)));
		commonBugs.put("NamedGroup Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_NAMED_GROUP_INTOLERANCE)));
		commonBugs.put("Empty last Extension Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_EMPTY_LAST_EXTENSION_INTOLERANCE)));
		commonBugs.put("SigHashAlgo Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_SIG_HASH_ALGORITHM_INTOLERANCE)));
		commonBugs.put("Big ClientHello Intolerant",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_BIG_CLIENT_HELLO_INTOLERANCE)));
		commonBugs.put("2nd Ciphersuite Byte Bug",
				String.valueOf(report.getResult(AnalyzedProperty.HAS_SECOND_CIPHER_SUITE_BYTE_BUG)));
		commonBugs.put("Ignores offered Ciphersuites",
				String.valueOf(report.getResult(AnalyzedProperty.IGNORES_OFFERED_CIPHER_SUITES)));
		commonBugs.put("Reflects offered Ciphersuites",
				String.valueOf(report.getResult(AnalyzedProperty.REFLECTS_OFFERED_CIPHER_SUITES)));
		commonBugs.put("Ignores offered NamedGroups",
				String.valueOf(report.getResult(AnalyzedProperty.IGNORES_OFFERED_NAMED_GROUPS)));
		commonBugs.put("Ignores offered SigHashAlgos",
				String.valueOf(report.getResult(AnalyzedProperty.IGNORES_OFFERED_SIG_HASH_ALGOS)));
	...
	public ObjectNode appendAttackVulnerabilities(ObjectNode jsonReport) {
		...
		if (report.getKnownVulnerability() == null) {
			attackVulnerabilities.put("Padding Oracle",
					String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_PADDING_ORACLE)));
		} else {
			attackVulnerabilities.put("Padding Oracle", "true - " + report.getKnownVulnerability().getShortName());
		}
		attackVulnerabilities.put("Bleichenbacher",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_BLEICHENBACHER)));
		attackVulnerabilities.put("Raccoon",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_RACCOON_ATTACK)));
		attackVulnerabilities.put("Direct Raccoon",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_DIRECT_RACCOON)));
		...
		attackVulnerabilities.put("Invalid Curve",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_INVALID_CURVE)));
		attackVulnerabilities.put("Invalid Curve (ephemeral)",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_INVALID_CURVE_EPHEMERAL)));
		attackVulnerabilities.put("Invalid Curve (twist)",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_INVALID_CURVE_TWIST)));
		attackVulnerabilities.put("SSL Poodle",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_POODLE)));
		attackVulnerabilities.put("TLS Poodle",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_TLS_POODLE)));
		attackVulnerabilities.put("Logjam", String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_LOGJAM)));
		attackVulnerabilities.put("Sweet 32",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_SWEET_32)));
		attackVulnerabilities.put("General DROWN",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_GENERAL_DROWN)));
		attackVulnerabilities.put("Extra Clear DROWN",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_EXTRA_CLEAR_DROWN)));
		attackVulnerabilities.put("Heartbleed",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_HEARTBLEED)));
		attackVulnerabilities.put("EarlyCcs",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_EARLY_CCS)));
		attackVulnerabilities.put("CVE-2020-13777 (Zero key)",
				String.valueOf(report.getResult(AnalyzedProperty.VULNERABLE_TO_SESSION_TICKET_ZERO_KEY)));
		...
		if ((report.getResult(AnalyzedProperty.VULNERABLE_TO_RACCOON_ATTACK) == TestResult.TRUE
			...
			if (report.getRaccoonAttackProbabilities().size() > 0) {
				...
				if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					...
					raccoon.put("PSK Length Probabilties",
							addIndentations("PSK Length") + addIndentations("Bitleak") + "Probability");
		...
		if (informationLeakTestList == null || informationLeakTestList.isEmpty()) {
			...
			for (InformationLeakTest testResult : informationLeakTestList) {
				...
				if (testResult.getValueP() < 0.01) {
					...
					node.put("result", padToLength(resultString, 80) + " | " + padToLength("No behavior difference", 25)
							+ padToLength("| NOT VULNERABLE", 25) + "| P: " + pValue);
				}
		...
		try {
			...
			if (Objects.equals(report.getResult(AnalyzedProperty.VULNERABLE_TO_PADDING_ORACLE), TestResult.TRUE)) {
				if (report.getKnownVulnerability() != null) {
					...
					paddingOracle.put("Identification",
	...
	public ObjectNode appendEcPointFormats(ObjectNode jsonReport) {
		...
		ellipticCurvePointFormats.put("Uncompressed",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_UNCOMPRESSED_POINT)));
		ellipticCurvePointFormats.put("ANSIX962 Prime",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ANSIX962_COMPRESSED_PRIME)));
		ellipticCurvePointFormats.put("ANSIX962 Char2",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ANSIX962_COMPRESSED_CHAR2)));
		ellipticCurvePointFormats.put("TLS 1.3 ANSIX962  SECP",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS13_SECP_COMPRESSION)));
			...
			for (InvalidCurveResponse response : report.getInvalidCurveResultList()) {
				...
				if ((response.getShowsVulnerability() == TestResult.TRUE
					...
					case TRUE:
						...
						break;
					case FALSE:
						invalidCurveResponse.put("pointValidation",
	...
	public ObjectNode appendCipherSuites(ObjectNode jsonReport) {
		if (report.getCipherSuites() != null) {
			...
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
				...
				symmetricSupported.put("Null",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_NULL_CIPHERS)));
				...
				symmetricSupported.put("CAMELLIA",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CAMELLIA)));
				symmetricSupported.put("ARIA", String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ARIA)));
				symmetricSupported.put("CHACHA20 POLY1305",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CHACHA)));
				...
				keyExchangeSupported.put("Kerberos",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_KERBEROS)));
				keyExchangeSupported.put("Plain PSK",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_PSK_PLAIN)));
				keyExchangeSupported.put("PSK RSA",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_PSK_RSA)));
				keyExchangeSupported.put("PSK DHE",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_PSK_DHE)));
				keyExchangeSupported.put("PSK ECDHE",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_PSK_ECDHE)));
				keyExchangeSupported.put("Fortezza",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_FORTEZZA)));
				keyExchangeSupported.put("New Hope",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_NEWHOPE)));
				keyExchangeSupported.put("ECMQV", String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ECMQV)));
				keyExchangeSupported.put("TLS 1.3 PSK_DHE",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS13_PSK_DHE)));
				...
				cipherTypesSupports.put("Stream",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_STREAM_CIPHERS)));
				cipherTypesSupports.put("Block",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_BLOCK_CIPHERS)));
			...
			perfectForwardSecrecy.put("Supports Only PFS",
					String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ONLY_PFS)));
			jsonReport.set("perfectForwardSecrecy", perfectForwardSecrecy);

			ObjectNode ciphersuiteGeneral = mapper.createObjectNode();
			ciphersuiteGeneral.put("Enforces Ciphersuite ordering",
					String.valueOf(report.getResult(AnalyzedProperty.ENFORCES_CS_ORDERING)));
	...
	public ObjectNode appendProtocolVersions(ObjectNode jsonReport) {
		if (report.getVersions() != null) {
			...
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
				...
				versions.put("TLS 1.3 Draft 14",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_14)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_15) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 15",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_15)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_16) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 16",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_16)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_17) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 17",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_17)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_18) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 18",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_18)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_19) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 19",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_19)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_20) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 20",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_20)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_21) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 21",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_21)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_22) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 22",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_22)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_23) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 23",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_23)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_24) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 24",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_24)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_25) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 25",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_25)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_26) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 26",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_26)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_27) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 27",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_27)));
			}
			if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)
					|| report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_28) == TestResult.TRUE) {
				versions.put("TLS 1.3 Draft 28",
						String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TLS_1_3_DRAFT_28)));
			}
		...
		if (report.getResult(AnalyzedProperty.SUPPORTS_HTTPS) == TestResult.TRUE) {
			try {
				...
				if (report.getResult(AnalyzedProperty.SUPPORTS_HSTS) == TestResult.TRUE) {
					...
					hsts.put("HSTS Preloading",
							String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_HSTS_PRELOADING)));
				...
				if (report.getResult(AnalyzedProperty.SUPPORTS_HPKP) == TestResult.TRUE
					...
					hpkp.put("HPKP (report only)",
							String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_HPKP_REPORTING)));
				...
				for (HttpsHeader header : report.getHeaderList()) {
					HTTPSResponseHeader
							.add(header.getHeaderName().getValue() + ":" + header.getHeaderValue().getValue());
				}
			} catch (Exception E) {
	...
	public ObjectNode appendExtensions(ObjectNode jsonReport) {
		...
		extensionDetails.put("Secure Renegotiation",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_SECURE_RENEGOTIATION_EXTENSION)));
		extensionDetails.put("Extended Master Secret",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_EXTENDED_MASTER_SECRET)));
		extensionDetails.put("Encrypt Then Mac",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ENCRYPT_THEN_MAC)));
		extensionDetails.put("Tokenbinding", String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_TOKENBINDING)));
		extensionDetails.put("Certificate Status Request",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST)));
		extensionDetails.put("Certificate Status Request v2",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST_V2)));
	...
	public void appendPublicKeyIssues(ObjectNode jsonReport) {
		...
		publicKeyParameter.put("EC PublicKey reuse",
				String.valueOf(report.getResult(AnalyzedProperty.REUSES_EC_PUBLICKEY)));
		publicKeyParameter.put("DH PublicKey reuse",
				String.valueOf(report.getResult(AnalyzedProperty.REUSES_DH_PUBLICKEY)));
		publicKeyParameter.put("Uses Common DH Primes",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_COMMON_DH_PRIMES)));
		...
		publicKeyParameter.put("Uses only prime moduli",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ONLY_PRIME_MODULI)));
		publicKeyParameter.put("Uses only safe-prime moduli",
				String.valueOf(report.getResult(AnalyzedProperty.SUPPORTS_ONLY_SAFEPRIME_MODULI)));
			...
			influencers.entrySet().stream().sorted((o1, o2) -> {
				...
				if (influencer.isBadInfluence() || influencer.getReferencedProperty() != null) {
					...
					PropertyResultRecommendation resultRecommendation = recommendation
							.getPropertyResultRecommendation(influencer.getResult());
					if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
						recommendationNode
								.add(printFullRecommendation(rater, recommendation, influencer, resultRecommendation));
					} else {
				...
				for (NamedGroup group : report.getSupportedNamedGroups()) {
					...
					if (detail == ScannerDetail.ALL) {
						...
						if (witness.getEcdsaPkGroupEphemeral() != null && witness.getEcdsaPkGroupEphemeral() != group) {
							namedGroup.put("ECDSA Required Groups", witness.getEcdsaPkGroupEphemeral()
	...
	public ObjectNode appendSignatureAndHashAlgorithms(ObjectNode jsonReport) {
		if (report.getSupportedSignatureAndHashAlgorithms() != null) {
			ArrayNode supportedSignatureAndHashAlgorithms = mapper.createArrayNode();
			if (report.getSupportedSignatureAndHashAlgorithms().size() > 0) {
				for (SignatureAndHashAlgorithm algorithm : report.getSupportedSignatureAndHashAlgorithms()) {
					supportedSignatureAndHashAlgorithms.add(algorithm.toString());
				}
			}
			jsonReport.set("Supported Signature and Hash Algorithms", supportedSignatureAndHashAlgorithms);
		}
		return jsonReport;
	}
	...
	private ObjectNode sessionTicketZeroKeyDetails(ObjectNode jsonReport) {
		...
		if (report.getResult(AnalyzedProperty.VULNERABLE_TO_SESSION_TICKET_ZERO_KEY) == TestResult.TRUE) {
			sessionTicketZeroAttack.put("Has GnuTls magic bytes:",
					String.valueOf(report.getResult(AnalyzedProperty.HAS_GNU_TLS_MAGIC_BYTES)));
		}
