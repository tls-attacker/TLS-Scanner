...
public class CertificateProbe extends TlsProbe {
    ...
    //the curves for which the server presented a certificate
    //note that the server may use additional curves for ecdh key exchange
    private List<NamedGroup> ecdsaGroupsFromStatic;
    private List<NamedGroup> ecdsaGroupsFromEphemeral;
    ...
    public ProbeResult executeTest() {
        try {
            ...
            {
                return new CertificateResult(certificates);
            }
        } catch (Exception E) {
    ...
    private List<CertificateChain> getEcdsaCerts()
    {
        ...
        performEcCertScan(tlsConfig, cipherSuitesToTest, ecdsaCerts);
    ...
    private List<CertificateChain> getDssCerts()
    {
        ...
        newCert = performCertScan(tlsConfig, cipherSuitesToTest);
        if(newCert != null && certAlreadyInList(dssCerts, newCert)) {
            dssCerts.add(newCert);
        }
        return dssCerts;
    }
    ...
    private CertificateChain performCertScan(Config tlsConfig, List<CipherSuite> cipherSuitesToTest)
    {
        ...
        executeState(state);
        if(WorkflowTraceUtil.didReceiveMessage(HandshakeMessageType.SERVER_HELLO_DONE, state.getWorkflowTrace()) && cipherSuitesToTest.contains(state.getTlsContext().getSelectedCipherSuite()) && state.getTlsContext().getServerCertificate() != null)
        {
           return new CertificateChain(state.getTlsContext().getServerCertificate(), tlsConfig.getDefaultClientConnection().getHostname()); 
        }
        else
        {
            return null;
    ...
    private void performEcCertScan(Config tlsConfig, List<CipherSuite> cipherSuitesToTest, List<CertificateChain> certificateList) {
        tlsConfig.setDefaultClientSupportedCiphersuites(cipherSuitesToTest);
        ...
        {   
            State state = new State(tlsConfig); 
            executeState(state);  
            if(WorkflowTraceUtil.didReceiveMessage(HandshakeMessageType.SERVER_HELLO_DONE, state.getWorkflowTrace()) && cipherSuitesToTest.contains(state.getTlsContext().getSelectedCipherSuite()) && state.getTlsContext().getServerCertificate() != null && state.getTlsContext().getEcCertificateCurve() != null && groupsToTest.contains(state.getTlsContext().getEcCertificateCurve()))
            {
                groupsToTest.remove(state.getTlsContext().getEcCertificateCurve());
                certificateList.add(new CertificateChain(state.getTlsContext().getServerCertificate(), tlsConfig.getDefaultClientConnection().getHostname())); 
            }
            else
            {
    ...
    private boolean isSameCertificate(CertificateChain cert1, CertificateChain cert2)
    {
        ...
        else
        {
            ...
            {
                if(cert1.getCertificate().getCertificateList()[i].getSerialNumber().equals(cert1.getCertificate().getCertificateList()[i].getSerialNumber()) == false)
                {
                    return false;
                }
            }
