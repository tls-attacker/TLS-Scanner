...
public class ServerContainerReportCreator extends TlsReportCreator {
    ...
    public ReportContainer createReport(ServerReport report) {
        ...
        container
            .add(new TextContainer("Report for " + report.getHost() + ":" + report.getPort(), AnsiColor.DEFAULT_COLOR));
        if (report.getServerIsAlive() == Boolean.FALSE) {
            container.add(new TextContainer("Cannot reach the Server. Is it online?", AnsiColor.DEFAULT_COLOR));
            return container;
        }
        if (report.getSpeaksProtocol() == Boolean.FALSE) {
            container.add(new TextContainer(
    ...
    protected ReportContainer createDtlsPortContainer(ServerReport report) {
        ...
        if (report.getResult(TlsAnalyzedProperty.CHANGES_PORT) == TestResults.TRUE) {
            container.add(createKeyValueContainer(TlsAnalyzedProperty.CHANGES_PORT_TO_RANDOM_PORTS, report));
        }
        return container;
    }
    ...
    protected ReportContainer createDtlsCookieContainer(ServerReport report) {
        ...
        if (report.getCookieLength() != null) {
            container.add(createDefaultKeyValueContainer("Cookie length", report.getCookieLength().toString()));
        }
        container.add(createKeyValueContainer(TlsAnalyzedProperty.USES_IP_ADDRESS_FOR_COOKIE, report));
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.USES_SESSION_ID_FOR_COOKIE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.USES_CIPHERSUITES_FOR_COOKIE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.USES_COMPRESSIONS_FOR_COOKIE, report));
        return container;
    }
    ...
    private ReportContainer createExtensionsContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_SECURE_RENEGOTIATION_EXTENSION, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_EXTENDED_MASTER_SECRET, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_ENCRYPT_THEN_MAC, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_TOKENBINDING, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_CERTIFICATE_STATUS_REQUEST_V2, report));
                ...
                for (NamedGroup group : report.getSupportedNamedGroups()) {
                    ...
                    if (detail == ScannerDetail.ALL) {
                        ...
                        if (witness.getEcdsaPkGroupEphemeral() != null && witness.getEcdsaPkGroupEphemeral() != group) {
                            curveDetails.add(createDefaultTextContainer(witness.getEcdsaPkGroupEphemeral()
                        ...
                        if (witness.getEcdsaSigGroupEphemeral() != null
                            ...
                            curveDetails.add(createDefaultTextContainer(witness.getEcdsaSigGroupEphemeral()
                                + " (Certificate Signature  - Ephemeral Cipher Suite)"));
                        }
                        if (witness.getEcdsaSigGroupStatic() != null && witness.getEcdsaSigGroupStatic() != group) {
                            curveDetails.add(createDefaultTextContainer(
                                witness.getEcdsaSigGroupStatic() + " (Certificate Signature  - Static Cipher Suite)"));
                        }
                    }
                }
                if (report.getResult(TlsAnalyzedProperty.GROUPS_DEPEND_ON_CIPHER) == TestResults.TRUE) {
                    container.add(new TextContainer("Not all Groups are supported for all Cipher Suites",
                        AnsiColor.DEFAULT_COLOR));
                }
                if (report.getResult(TlsAnalyzedProperty.IGNORES_ECDSA_GROUP_DISPARITY) == TestResults.TRUE) {
                    container.add(
                        new TextContainer("Groups required for ECDSA validation are not enforced", AnsiColor.YELLOW));
                }
            } else {
                container.add(new TextContainer("none", AnsiColor.DEFAULT_COLOR));
            }
    ...
    private ReportContainer createEcPointFormatsContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_UNCOMPRESSED_POINT, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_ANSIX962_COMPRESSED_PRIME, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_ANSIX962_COMPRESSED_CHAR2, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_TLS13_SECP_COMPRESSION, report));
        return container;
    }
    ...
    private ReportContainer createIntolerancesContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_CIPHER_SUITE_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_EXTENSION_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_CIPHER_SUITE_LENGTH_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_COMPRESSION_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_ALPN_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_CLIENT_HELLO_LENGTH_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_NAMED_GROUP_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_EMPTY_LAST_EXTENSION_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_SIG_HASH_ALGORITHM_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_BIG_CLIENT_HELLO_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_SECOND_CIPHER_SUITE_BYTE_BUG, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.IGNORES_OFFERED_CIPHER_SUITES, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.REFLECTS_OFFERED_CIPHER_SUITES, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.IGNORES_OFFERED_NAMED_GROUPS, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.IGNORES_OFFERED_SIG_HASH_ALGOS, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_GREASE_CIPHER_SUITE_INTOLERANCE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_GREASE_NAMED_GROUP_INTOLERANCE, report));
        container.add(
            createKeyValueContainer(TlsAnalyzedProperty.HAS_GREASE_SIGNATURE_AND_HASH_ALGORITHM_INTOLERANCE, report));
        return container;
    }
    ...
    private ReportContainer createHelloRetryContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SENDS_HELLO_RETRY_REQUEST, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.ISSUES_COOKIE_IN_HELLO_RETRY, report));
        return container;
    }
    ...
    private ReportContainer createAttackVulnerabilitiesContainer(ServerReport report) {
        ...
        if (report.getKnownPaddingOracleVulnerability() == null) {
            container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_PADDING_ORACLE, report));
        } else {
            container.add(new KeyValueContainer("Padding Oracle", AnsiColor.DEFAULT_COLOR,
                report.getKnownPaddingOracleVulnerability().getShortName(), AnsiColor.RED));
        }
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_BLEICHENBACHER, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_RACCOON_ATTACK, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_DIRECT_RACCOON, report));
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_INVALID_CURVE, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_INVALID_CURVE_EPHEMERAL, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_INVALID_CURVE_TWIST, report));
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_GENERAL_DROWN, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_EXTRA_CLEAR_DROWN, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_HEARTBLEED, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_EARLY_CCS, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_SESSION_TICKET_ZERO_KEY, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.ALPACA_MITIGATED, report));
        container
            .add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_RENEGOTIATION_ATTACK_EXTENSION_V1, report));
        container
            .add(createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_RENEGOTIATION_ATTACK_EXTENSION_V2, report));
        container.add(
            createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_RENEGOTIATION_ATTACK_CIPHERSUITE_V1, report));
        container.add(
            createKeyValueContainer(TlsAnalyzedProperty.VULNERABLE_TO_RENEGOTIATION_ATTACK_CIPHERSUITE_V2, report));
        return container;
    }
    ...
    private ReportContainer createAlpnContainer(ServerReport report) {
        ...
        for (AlpnProtocol alpnProtocol : AlpnProtocol.values()) {
            ...
            if (report.getSupportedAlpns().contains(alpnProtocol.getConstant())) {
                container.add(new KeyValueContainer(alpnProtocol.getPrintableName(), AnsiColor.DEFAULT_COLOR, "true",
                    AnsiColor.DEFAULT_COLOR));
            } else {
                if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
                    container.add(new KeyValueContainer(alpnProtocol.getPrintableName(), AnsiColor.DEFAULT_COLOR,
                        "false", AnsiColor.DEFAULT_COLOR));
                }
            }
        }
    ...
    private ReportContainer createSessionTicketZeroKeyContainer(ServerReport report) {
        ...
        if (report.getResult(TlsAnalyzedProperty.VULNERABLE_TO_SESSION_TICKET_ZERO_KEY) == TestResults.TRUE) {
            ...
            container.add(createKeyValueContainer(TlsAnalyzedProperty.HAS_GNU_TLS_MAGIC_BYTES, report));
        }
        return container;
    }
    ...
    private ReportContainer createCertificateTransparencyContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_SCTS_PRECERTIFICATE, report));
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_CHROME_CT_POLICY, report));
        ...
        if (report.getResult(TlsAnalyzedProperty.SUPPORTS_HTTPS) == TestResults.TRUE) {
            ...
            try {
                ...
                if (report.getResult(TlsAnalyzedProperty.SUPPORTS_HSTS) == TestResults.TRUE) {
                    container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_HSTS, report));
                    container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_HSTS_PRELOADING, report));
                    container.add(
                        createDefaultKeyValueContainer("max-age (seconds)", String.valueOf(report.getHstsMaxAge())));
                } else {
                ...
                if (report.getResult(TlsAnalyzedProperty.SUPPORTS_HPKP) == TestResults.TRUE
                    ...
                    container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_HPKP, report));
                    container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_HPKP_REPORTING, report));
                    container.add(
                        createDefaultKeyValueContainer("max-age (seconds)", String.valueOf(report.getHpkpMaxAge())));
                    if (!report.getNormalHpkpPins().isEmpty()) {
                        container.add(new KeyValueContainer("HPKP-Pins", AnsiColor.DEFAULT_COLOR,
                            report.getNormalHpkpPins().toString(), AnsiColor.GREEN));
                    }
                    if (!report.getReportOnlyHpkpPins().isEmpty()) {
                        container.add(new KeyValueContainer("Report Only HPKP-Pins", AnsiColor.DEFAULT_COLOR,
                            report.getReportOnlyHpkpPins().toString(), AnsiColor.GREEN));
                    }
                ...
                container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_HTTP_FALSE_START, report));
            } catch (Exception e) {
    ...
    private ReportContainer createPublicKeyIssuesContainer(ServerReport report) {
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_COMMON_DH_PRIMES, report));
        ...
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_ONLY_PRIME_MODULI, report));
        container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_ONLY_SAFEPRIME_MODULI, report));
        if (report.getWeakestDhStrength() != null) {
            if (report.getWeakestDhStrength() < 1000) {
                container.add(new KeyValueContainer("DH Strength", AnsiColor.DEFAULT_COLOR,
                    String.valueOf(report.getWeakestDhStrength()), AnsiColor.RED));
            } else if (report.getWeakestDhStrength() < 2000) {
                container.add(new KeyValueContainer("DH Strength", AnsiColor.DEFAULT_COLOR,
                    String.valueOf(report.getWeakestDhStrength()), AnsiColor.YELLOW));
            } else if (report.getWeakestDhStrength() < 4100) {
                container.add(new KeyValueContainer("DH Strength", AnsiColor.DEFAULT_COLOR,
                    String.valueOf(report.getWeakestDhStrength()), AnsiColor.GREEN));
            } else {
                container.add(new KeyValueContainer("DH Strength", AnsiColor.DEFAULT_COLOR,
                    String.valueOf(report.getWeakestDhStrength()), AnsiColor.YELLOW));
            }
    ...
    private ReportContainer createClientAuthenticationContainer(ServerReport report) {
        ...
        container.add(createDefaultKeyValueContainer("Supported", String.valueOf(report.getCcaSupported())));
        container.add(createDefaultKeyValueContainer("Required", String.valueOf(report.getCcaRequired())));

        if (report.getCcaTestResultList() != null) {
            List<CcaTestResult> ccaTestResults = report.getCcaTestResultList();
            ccaTestResults.sort((ccaTestResult, t1) -> {
                int c;
            ...
            for (CcaTestResult ccaTestResult : ccaTestResults) {
                ...
                if (ccaTestResult.getCertificateType() != lastCcaCertificateType) {
                    ...
                    ccaTestResultsContainer.add(new HeadlineContainer(lastCcaCertificateType.name()));
                }
                ...
                container.add(new KeyValueContainer(
                    ...
                    AnsiColor.DEFAULT_COLOR, String.valueOf(ccaTestResult.getSucceeded()),
    ...
    private ReportContainer createScoringResultsContainer(ServerReport report) {
        ...
        try {
            ...
            container.add(createDefaultKeyValueContainer("Score", String.valueOf(scoreReport.getScore())));
            if (!detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
                return container;
            }
            scoreReport.getInfluencers().forEach((key, influencer) -> {
                Recommendation recommendation = rater.getRecommendations().getRecommendation(key);
        ...
        try {
            ...
            influencers.entrySet().stream().sorted(Map.Entry.comparingByValue()).forEach((entry) -> {
                ...
                if (influencer.isBadInfluence() || influencer.getReferencedProperty() != null) {
                    ...
                    if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
                        printFullRecommendation(container, rater, recommendation, influencer, resultRecommendation);
                    } else {
                        printShortRecommendation(container, influencer, resultRecommendation);
                    }
                }
            });
        } catch (Exception ex) {
            container.add(new TextContainer("Could not append recommendations - unrelated error", AnsiColor.RED));
            LOGGER.error("Could not append recommendations", ex);
        }
        return container;
    }

    private void printFullRecommendation(ListContainer outerContainer, SiteReportRater rater,
        Recommendation recommendation, PropertyResultRatingInfluencer influencer,
        PropertyResultRecommendation resultRecommendation) {
        AnsiColor color = getColorForRecommendation(influencer);
        outerContainer.add(new KeyValueContainer(recommendation.getShortName(), AnsiColor.DEFAULT_COLOR,
            influencer.getResult().name(), color));
        int scoreInfluence = 0;
        ...
        outerContainer
            .add(new KeyValueContainer("Score", AnsiColor.DEFAULT_COLOR, scoreInfluence + additionalInfo, color));
        if (influencer.hasScoreCap()) {
            outerContainer.add(new KeyValueContainer("Score cap", AnsiColor.DEFAULT_COLOR,
                influencer.getScoreCap().toString(), color));
        }
        outerContainer.add(new KeyValueContainer("Information", AnsiColor.DEFAULT_COLOR,
            resultRecommendation.getShortDescription(), color));
        outerContainer.add(new KeyValueContainer("Recommendation", AnsiColor.DEFAULT_COLOR,
            resultRecommendation.getHandlingRecommendation(), color));
    }

    private void printShortRecommendation(ListContainer outerContainer, PropertyResultRatingInfluencer influencer,
        PropertyResultRecommendation resultRecommendation) {
        AnsiColor color = getColorForRecommendation(influencer);
        outerContainer.add(new TextContainer(
    ...
    public ReportContainer createPerformanceDataContainer(ServerReport report) {
        ...
        if (detail.isGreaterEqualTo(ScannerDetail.ALL)) {
            ...
            try {
                container.add(createDefaultKeyValueContainer("TCP connections",
                ...
                for (PerformanceData data : report.getPerformanceList()) {
                    ...
                    performance.add(createDefaultKeyValueContainer(data.getType().getName(),
