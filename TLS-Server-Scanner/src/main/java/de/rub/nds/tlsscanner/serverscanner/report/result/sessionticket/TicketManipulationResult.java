...
public class TicketManipulationResult extends VersionDependentResult {
    ...
    public TicketManipulationResult(ProtocolVersion protocolVersion, TestResult overallResult,
        Map<Integer, VectorResponse> responses, ResponseFingerprint acceptFingerprint,
        ResponseFingerprint acceptDifferentSecretFingerprint, ResponseFingerprint rejectFingerprint) {
    ...
    public void writeToSiteReport(SiteReport report) {
        putResult(report, AnalyzedProperty.NO_MAC_CHECK_TICKET, overallResult, true);
    }
    ...
    public String getResultsAsShortString(Map<ResponseFingerprint, Integer> classifications, boolean collapseBytes) {
        ...
        for (int byteIndex = 0; byteIndex <= maxByte; byteIndex++) {
            ...
            if (collapseBytes) {
                res.append(getResultAsShortStringForRangeCollapsed(byteIndex * 8, byteIndex * 8 + 8, classifications));
            } else {
                getResultAsShortStringForRange(res, byteIndex * 8, byteIndex * 8 + 8, classifications);
                res.append(' ');
            }
        }
        return res.toString();
    ...
    private void getResultAsShortStringForRange(StringBuilder res, int firstBit, int exclusiveLastBit,
        ...
        for (int i = firstBit; i < exclusiveLastBit; i++) {
            ...
            if (fingerprint == null) {
                ...
                if (classifications != null) {
                    classification = classifications.computeIfAbsent(fingerprint, k -> classifications.size());
                }
    ...
    private char getResultAsShortStringForRangeCollapsed(int firstBit, int exclusiveLastBit,
        Map<ResponseFingerprint, Integer> classifications) {
