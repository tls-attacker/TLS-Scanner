...
public class PrefixStatsUtil {
    ...
    /**
     * Compute after which prefix length how many divergences happen. I.e. build a prefix tree and count divergences at
     * each level.
     *
     * The strings "AAB", "AAC", "AAD" would find 3 divergences after depth 2; this means that the prefix will most
     * likely have the length 2
     *
     * @param  strings
     *                 (Byte)strings to analyze for common prefixes
     * @return         A map mapping {prefix length}->{divergences after this length}
     */
    ...
    private static void computePrefixDivergences(List<byte[]> strings, Map<Integer, Integer> result,
        byte[] prefixThusFar) {
        ...
        // however, we don't really build a tree, but only remember the (sumed up) degree in each depth
        ...
        result.compute(prefixThusFar.length, (k, v) -> (v == null ? 0 : v) + nextPrefixes.keySet().size());
        // DFS for divergences
