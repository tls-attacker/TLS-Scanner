...
public class ProtocolVersionResult<Report extends TlsScanReport> extends ProbeResult<Report> {
    ...
    public void mergeData(TlsScanReport report) {
        if (supportedProtocolVersions != null) {
            ...
            for (ProtocolVersion version : unsupportedProtocolVersions) {
                if (version == ProtocolVersion.DTLS10_DRAFT) {
                    report.putResult(
                            TlsAnalyzedProperty.SUPPORTS_DTLS_1_0_DRAFT, TestResults.FALSE);
                }
                if (version == ProtocolVersion.DTLS10) {
            ...
            report.putResult(
                    TlsAnalyzedProperty.SUPPORTS_DTLS_1_0_DRAFT, TestResults.COULD_NOT_TEST);
