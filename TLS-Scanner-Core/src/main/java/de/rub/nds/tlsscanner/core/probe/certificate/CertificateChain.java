...
public class CertificateChain {
    ...
    public CertificateChain(Certificate certificate, String uri) {
        ...
        if (leafReport != null) {
            if (certificateReportList.isEmpty()
                    || !certificateReportList
                            .get(0)
                            .getSHA256Fingerprint()
                            .equals(leafReport.getSHA256Fingerprint())) {
            ...
            while (!tempCertificate.getIssuer().equals(tempCertificate.getSubject())) {
                ...
                if (foundReport != null) {
                    ...
                    if (TrustAnchorManager.getInstance().isInitialized()) {
                        if (TrustAnchorManager.getInstance()
                            ...
                            if (trustAnchorCertificate != null) {
                                CertificateReport trustAnchorReport =
                                        CertificateReportGenerator.generateReport(
                        ...
                        LOGGER.error(
                                "Cannot check if the chain is complete since the trust manager is not "
                                        + "initialized");
                    }
                    break;
                }
        ...
        for (CertificateReport report : certificateReportList) {
            ...
            if (report.getSignatureAndHashAlgorithm() != null) {
                if (Objects.equals(report.isTrustAnchor(), Boolean.FALSE)
                                ...
                                && report.getSignatureAndHashAlgorithm().getHashAlgorithm()
                                        == HashAlgorithm.MD5
                        || report.getSignatureAndHashAlgorithm().getHashAlgorithm()
                                == HashAlgorithm.SHA1) {
        ...
        if (Objects.equals(chainIsComplete, Boolean.TRUE)
                && Objects.equals(containsValidLeaf, Boolean.TRUE)
                && Objects.equals(containsExpired, Boolean.FALSE)
                && Objects.equals(containsNotYetValid, Boolean.FALSE)
                && Objects.equals(containsCustomTrustAnchor, Boolean.FALSE)) {
            CertPathValidationResult certPathValidationResult =
                    evaluateGeneralTrust(orderedCertificateChain);
                        ...
                        if (exception.getMessage().contains("Unhandled Critical Extensions")
                                || (exception.getCause() != null
                                        ...
                                        && exception
                                                .getCause()
                                                .getMessage()
                                                .contains("Unhandled Critical Extensions"))) {
    ...
    private CertPathValidationResult evaluateGeneralTrust(
            List<CertificateReport> orderedCertificateChain) {
        if (orderedCertificateChain.size() < 2) {
            return null; // Emtpy chains & only root ca's are not considered
            // generally trusted i guess
        }
        X509CertificateHolder[] certPath =
                new X509CertificateHolder[orderedCertificateChain.size()];
        for (int i = 0; i < orderedCertificateChain.size(); i++) {
            certPath[i] =
                    new X509CertificateHolder(orderedCertificateChain.get(i).getCertificate());
        }
