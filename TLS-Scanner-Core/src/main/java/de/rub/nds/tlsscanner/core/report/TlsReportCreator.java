...
public class TlsReportCreator extends ReportCreator {
	...
	protected ReportContainer createCompressionContainer(TlsScanReport report) {
		...
		if (report.getSupportedCompressionMethods() != null) {
			...
			for (CompressionMethod compression : report.getSupportedCompressionMethods()) {
				container.add(new TextContainer(compression.name(), getColorForCompression(compression)));
			}
		}
		return container;
	}
		...
		if (report.getVersionSuitePairs() != null) {
			for (VersionSuiteListPair pair : report.getVersionSuitePairs()) {
				String enforcesCsOrderingSuffix = report
						.getResult(TlsAnalyzedProperty.ENFORCES_CS_ORDERING) == TestResults.TRUE ? "(server order)"
								: "";
				String versionHeadline = "Supported in " + pair.getVersion().toHumanReadable()
				...
				for (CipherSuite suite : pair.getCipherSuiteList()) {
					versionSuites.add(new TextContainer(suite.name(), getColorForCipherSuite(suite)));
				}
	...
	protected ReportContainer createRecordFragmentationContainer(TlsScanReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_RECORD_FRAGMENTATION, report));
		return container;
	}
	...
	protected ReportContainer createDtlsFragmenatationContainer(TlsScanReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_DTLS_FRAGMENTATION, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.DTLS_FRAGMENTATION_REQUIRES_EXTENSION, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.SUPPORTS_DTLS_FRAGMENTATION_WITH_INDIVIDUAL_PACKETS,
				report));
		container.add(createKeyValueContainer(
				TlsAnalyzedProperty.DTLS_FRAGMENTATION_WITH_INDIVIDUAL_PACKETS_REQUIRES_EXTENSION, report));
		return container;
	}
	...
	protected ReportContainer createDtlsRetransmissionsContainer(TlsScanReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.PROCESSES_RETRANSMISSIONS, report));
		container.add(createDefaultKeyValueContainer("Total retransmissions received",
	...
	protected ReportContainer createDtlsBugsContainer(TlsScanReport report) {
		...
		container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_UNENCRYPTED_FINISHED, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_UNENCRYPTED_APP_DATA, report));
		return container;
	}
	...
	protected ReportContainer createDtlsMessageSequenceNumberContainer(TlsScanReport report) {
		...
		container.add(
				createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_STARTED_WITH_INVALID_MESSAGE_SEQUENCE, report));
		container.add(createKeyValueContainer(TlsAnalyzedProperty.MISSES_MESSAGE_SEQUENCE_CHECKS, report));
		if (detail.isGreaterEqualTo(ScannerDetail.DETAILED)) {
			container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_SKIPPED_MESSAGE_SEQUENCES_ONCE, report));
			container.add(
					createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_SKIPPED_MESSAGE_SEQUENCES_MULTIPLE, report));
			container.add(createKeyValueContainer(TlsAnalyzedProperty.ACCEPTS_RANDOM_MESSAGE_SEQUENCES, report));
		}
		return container;
	}
	...
	protected ReportContainer createCertificateContainer(TlsScanReport report) {
		ListContainer container = new ListContainer();
		int certCtr = 1;
		if (report.getCertificateChainList() != null && !report.getCertificateChainList().isEmpty()) {
			for (CertificateChain chain : report.getCertificateChainList()) {
				container.add(new HeadlineContainer("Certificate Chain (Certificate " + certCtr + " of "
	...
	private void appendCertificate(ListContainer outerContainer, CertificateChain chain) {
		ListContainer container = new ListContainer();
		container.add(new KeyValueContainer("Chain ordered", AnsiColor.DEFAULT_COLOR,
				String.valueOf(chain.getChainIsOrdered()),
				chain.getChainIsOrdered() ? AnsiColor.GREEN : AnsiColor.YELLOW));
		container.add(new KeyValueContainer("Contains Trust Anchor", AnsiColor.DEFAULT_COLOR,
				String.valueOf(chain.getContainsTrustAnchor()),
				chain.getContainsTrustAnchor() ? AnsiColor.RED : AnsiColor.GREEN));
		container.add(new KeyValueContainer("Generally Trusted", AnsiColor.DEFAULT_COLOR,
		...
		if (!chain.getCertificateReportList().isEmpty()) {
			for (int i = 0; i < chain.getCertificateReportList().size(); i++) {
				...
				if (certReport.getSubject() != null) {
					subCertificateContainer.add(createDefaultKeyValueContainer("Subject", certReport.getSubject()));
				}

				if (certReport.getIssuer() != null) {
					subCertificateContainer.add(createDefaultKeyValueContainer("Issuer", certReport.getIssuer()));
				}
				if (certReport.getValidFrom() != null) {
					if (certReport.getValidFrom().before(new Date())) {
						subCertificateContainer.add(new KeyValueContainer("Valid From", AnsiColor.DEFAULT_COLOR,
								certReport.getValidFrom().toString(), AnsiColor.GREEN));
					} else {
						subCertificateContainer.add(new KeyValueContainer("Valid From", AnsiColor.DEFAULT_COLOR,
								certReport.getValidFrom().toString() + " - NOT YET VALID", AnsiColor.RED));
					}
				}
				if (certReport.getValidTo() != null) {
					if (certReport.getValidTo().after(new Date())) {
						subCertificateContainer.add(new KeyValueContainer("Valid Till", AnsiColor.DEFAULT_COLOR,
								certReport.getValidTo().toString(), AnsiColor.GREEN));
					} else {
						subCertificateContainer.add(new KeyValueContainer("Valid Till", AnsiColor.DEFAULT_COLOR,
								certReport.getValidTo().toString() + " - EXPIRED", AnsiColor.RED));
					}
				}
				if (certReport.getValidFrom() != null && certReport.getValidTo() != null
					...
					if (days < 1) {
						subCertificateContainer.add(new TextContainer(
								"Expires in <1 day! This certificate expires very soon", AnsiColor.RED));
					} else if (days < 3) {
						subCertificateContainer.add(new TextContainer(
								"Expires in " + days + " days! This certificate expires very soon", AnsiColor.RED));
					} else if (days < 14) {
						subCertificateContainer.add(new TextContainer(
								"Expires in " + days + " days! This certificate expires soon", AnsiColor.YELLOW));
					} else if (days < 31) {
						subCertificateContainer
								.add(new TextContainer("Expires in " + days + " days.", AnsiColor.DEFAULT_COLOR));
					} else if (days < 730) {
						subCertificateContainer
								.add(new TextContainer("Expires in " + days + " days.", AnsiColor.GREEN));
					} else if (Objects.equals(certReport.getLeafCertificate(), Boolean.TRUE)) {
						subCertificateContainer.add(new TextContainer(
								"Expires in " + days + " days. This is usually too long for a leaf certificate",
								AnsiColor.RED));
					} else {
						subCertificateContainer
								.add(new TextContainer("Expires in " + days / 365 + " years.", AnsiColor.GREEN));
					}
				}
				...
				if (certReport.getWeakDebianKey() != null) {
					subCertificateContainer.add(new KeyValueContainer("Weak Debian Key", AnsiColor.DEFAULT_COLOR,
				...
				if (certReport.getSignatureAndHashAlgorithm() != null) {
					subCertificateContainer.add(createDefaultKeyValueContainer("Signature Algorithm",
				...
				if (certReport.getSignatureAndHashAlgorithm() != null) {
					if (certReport.getSignatureAndHashAlgorithm().getHashAlgorithm() == HashAlgorithm.SHA1
						...
						if (!certReport.isTrustAnchor() && !certReport.getSelfSigned()) {
							subCertificateContainer.add(new KeyValueContainer("Hash Algorithm", AnsiColor.DEFAULT_COLOR,
									certReport.getSignatureAndHashAlgorithm().getHashAlgorithm().name(),
									AnsiColor.RED));
						} else {
							subCertificateContainer.add(createDefaultKeyValueContainer("Hash Algorithm",
									certReport.getSignatureAndHashAlgorithm().getHashAlgorithm().name()
											+ " - Not critical"));
						}
					} else {
				...
				if (certReport.getExtendedValidation() != null) {
					subCertificateContainer.add(new KeyValueContainer("Extended Validation", AnsiColor.DEFAULT_COLOR,
				...
				if (certReport.getCertificateTransparency() != null) {
					subCertificateContainer.add(new KeyValueContainer("Certificate Transparency",
							AnsiColor.DEFAULT_COLOR, String.valueOf(certReport.getCertificateTransparency()),
							certReport.getCertificateTransparency() ? AnsiColor.GREEN : AnsiColor.YELLOW));
				}

				if (certReport.getCrlSupported() != null) {
					subCertificateContainer.add(new KeyValueContainer("CRL Supported", AnsiColor.DEFAULT_COLOR,
				...
				if (certReport.getOcspSupported() != null) {
					subCertificateContainer.add(new KeyValueContainer("OCSP Supported", AnsiColor.DEFAULT_COLOR,
				...
				if (certReport.getOcspMustStaple() != null) {
					subCertificateContainer.add(createDefaultKeyValueContainer("OCSP must staple",
							String.valueOf(certReport.getOcspMustStaple())));
				}
				if (certReport.getRevoked() != null) {
					subCertificateContainer.add(new KeyValueContainer("RevocationStatus", AnsiColor.DEFAULT_COLOR,
							String.valueOf(certReport.getRevoked()),
							certReport.getRevoked() ? AnsiColor.RED : AnsiColor.GREEN));
				}
				if (certReport.getDnsCAA() != null) {
					subCertificateContainer.add(new KeyValueContainer("DNS CCA", AnsiColor.DEFAULT_COLOR,
				...
				if (certReport.getRocaVulnerable() != null) {
					subCertificateContainer.add(new KeyValueContainer("ROCA (simple)", AnsiColor.DEFAULT_COLOR,
							String.valueOf(certReport.getRocaVulnerable()),
							certReport.getRocaVulnerable() ? AnsiColor.RED : AnsiColor.GREEN));
				} else {
					subCertificateContainer.add(createDefaultTextContainer("ROCA (simple) not tested"));
				}
				subCertificateContainer
						.add(createDefaultKeyValueContainer("Fingerprint (SHA256)", certReport.getSHA256Fingerprint()));
	...
	private void appendPublicKey(ListContainer outerContainer, PublicKey publicKey) {
		if (publicKey instanceof CustomDhPublicKey) {
			...
			outerContainer.add(createDefaultKeyValueContainer("PublicKey Type:", "Static Diffie Hellman"));

			outerContainer.add(createDefaultKeyValueContainer("Modulus", dhPublicKey.getModulus().toString(16)));
			outerContainer.add(createDefaultKeyValueContainer("Generator", dhPublicKey.getModulus().toString(16)));
			outerContainer.add(createDefaultKeyValueContainer("Y", dhPublicKey.getY().toString(16)));
			...
			outerContainer.add(createDefaultKeyValueContainer("Modulus", dsaPublicKey.getDsaP().toString(16)));
			outerContainer.add(createDefaultKeyValueContainer("Generator", dsaPublicKey.getDsaG().toString(16)));
			outerContainer.add(createDefaultKeyValueContainer("Q", dsaPublicKey.getDsaQ().toString(16)));
			outerContainer.add(createDefaultKeyValueContainer("X", dsaPublicKey.getY().toString(16)));
			...
			outerContainer.add(createDefaultKeyValueContainer("Modulus", rsaPublicKey.getModulus().toString(16)));
			outerContainer.add(
					createDefaultKeyValueContainer("Public exponent", rsaPublicKey.getPublicExponent().toString(16)));
			...
			if (ecPublicKey.getGroup() == null) {
				outerContainer.add(createDefaultKeyValueContainer("Group (GOST)", ecPublicKey.getGostCurve().name()));
			} else {
				outerContainer.add(createDefaultKeyValueContainer("Group", ecPublicKey.getGroup().name()));
			}
			outerContainer.add(createDefaultKeyValueContainer("Public Point", ecPublicKey.getPoint().toString()));
		} else {
